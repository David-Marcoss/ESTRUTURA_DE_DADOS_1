...
void quant_digt(char mat[TAM][TAM][50] ,int *dig, int *mai){ //esta função mostra a quantidade de digitos e letras maiusculas das strings
	int i,linha,coluna;                                   // que estiverem na linha e coluna pasada pelo usuario
	...
	*mai = 0;
	...
	if ( (linha<0 || linha>TAM-1) || (coluna<0 || coluna>TAM-1) ){
		...
		for (i = 0; i < TAM; ++i){            //percorre a coluna e chama função digitos
			digitos(mat[i][coluna],dig,mai); //digitos retorna na variavel dig= quantidade de digitos da string
											// e na variavel mai quantidade de letras maiusculas			
		}
		printf("\nquantidade de digitos da matriz na coluna[%d ] : %d \n",coluna,*dig); //mostra o resultado para o usuario
		printf("quantidade de letras maiusculas da matriz na coluna [%d ] : %d \n\n",coluna,*mai);
		
		*dig = 0;
		*mai = 0;

		for (i = 0; i < TAM; ++i){         // agora percorre as linhas da matriz
			digitos(mat[linha][i],dig,mai);
		}
		printf("quantidade de digitos da matriz na linha [%d ] : %d \n",linha,*dig); //mostra o resultado para o usuario
		printf("quantidade de letras maiusculas da matriz na linha [%d ] : %d \n\n",linha,*mai);
	...
	while(op!=6){
		...
		switch (op){
			...
			case 4:
				...
				quant_digt(mat,&dig,&mai); 	 // esta opção ira mostrar a quantdade de digitos e letras maiusculas
